...
void InformesYGestion::visualizarVentas()
{
    ...
    {
        pedidoAnterior = "0";
        int cantidadDePedidosMostrados = 0, opcion;
        for (int i = 0; i < PeArchivos.cantidadDeVentasGuardadas(); i++)
        {
            if (cantidadDePedidosMostrados == 10)
            {
                cout << "------------------------------------" << endl;
                cout << "¨Desea vizualizar m s pedidos?" << endl;
                cout << "1 - S¡, 0 - No" << endl;
                cin >> opcion;
                if (opcion == 1)
                {
...
void InformesYGestion::recaudacionesMensuales(string anio)
{
    ...
    for (int i = 0; i < 12; i++)
    {
        ...
        cout << nombreMeses[i] <<" | $"<< fixed << setprecision(2) << meses[i] << endl;
    }
    delete[] listaPedidos;
    system("pause");
}
